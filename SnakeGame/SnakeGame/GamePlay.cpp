...
void GamePlay::Init() {
	...
	m_food.setPosition((int)m_context->m_window->getSize().x / 2, (int)m_context->m_window->getSize().y / 2);

	m_snake.Init(m_context->m_assets->GetTexture(SNAKE));

	m_scoreText.setFont(m_context->m_assets->GetFont(MAIN_FONT));
	m_scoreText.setPosition(16, 16);
	...
	if (!m_isPaused) {
		...
		if (m_elapsedTime.asSeconds() > 0.1) {
			...
			if (m_snake.IsOn(m_food)) {
				...
				/*
				x = clamp<int>(rand() % m_context->m_window->getSize().x, 16,
					rand() % m_context->m_window->getSize().x - 2*16);
				y = clamp<int>(rand() % m_context->m_window->getSize().y, 16,
					rand() % m_context->m_window->getSize().y - 2 * 16);
				*/
